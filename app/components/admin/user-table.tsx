...
import { useGetProgramasQuery } from "@/redux/features/campanias/campaniaApiSlice";
...
export function UserTable() {
  ...
  const [porDiplomado, setPorDiplomado] = useState<string | undefined>("");
  const [estatus, setEstatus] = useState<string>("");
  // const [open, setOpen] = useState<boolean>(false);
  const { q, page } = useAppSelector((state) => state.admin);
  const { data } = useObtainUsersQuery({ q, page, porDiplomado, estatus });
  ...
  const headers: ColumnDef<Estudiante>[] = [
    ...
    {
      header: "Estado del alumno",
      cell: ({ row }) => {
        const status = row.original.activo;
        return (
          <div className="">
            {status === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <div className="flex flex-col gap-1 w-full sm:w-auto">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-10 pr-4 py-2.5 h-11 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent w-full sm:w-full text-black"
                />
              </div>
            </div>
            ...
            <Select
              ...
              onChange={(e: ChangeEvent<HTMLSelectElement>) => {
                setPorDiplomado(e.target.value);
              }}
            />
