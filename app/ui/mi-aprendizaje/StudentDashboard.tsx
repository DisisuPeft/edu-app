...
export default function StudentDashboard(){
    const [courses, setCourses] = useState([
        ...
        },
        {
          ...
          progress: 85,
          completed: false,
          skills: ["Context API", "Hooks personalizados", "Optimización", "Testing"]
        },
        {
          id: 3,
      ...
      const certificatesEarned = courses.filter(course => course.certificate).length;
      const averageProgress = Math.round(courses.reduce((acc, course) => acc + course.progress, 0) / totalCourses);
    
      // Extraer todas las habilidades únicas
      const allSkills = Array.from(new Set(courses.flatMap(course => course.skills || [])));

      return (
        <div className="bg-gray-100 min-h-screen p-6">
            <div className="max-w-6xl mx-auto">
            <h1 className="text-3xl font-bold text-gray-800 mb-6">Mi aprendizaje</h1>
            
            {/* Tarjetas de estadísticas */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                <div className="bg-white p-6 rounded-lg shadow-md">
                <h2 className="text-gray-500 text-sm">Cursos Completados</h2>
                <p className="text-3xl font-bold">{completedCourses}/{totalCourses}</p>
                </div>
                <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            <div className="bg-white rounded-lg shadow-md overflow-hidden mb-8">
                ...
                <button 
                    ...
                    className={`py-3 px-6 font-medium ${activeTab === 'courses' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500'}`}
                >
                    Mis Cursos
                </button>
                <button 
                    onClick={() => setActiveTab('certificates')}
                    className={`py-3 px-6 font-medium ${activeTab === 'certificates' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500'}`}
                >
                    Mis Certificados
                </button>
                <button 
                    onClick={() => setActiveTab('skills')}
                    className={`py-3 px-6 font-medium ${activeTab === 'skills' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500'}`}
                >
                    Mis Habilidades
                </button>
                </div>
                    ...
                    {courses.map(course => (
                        <div key={course.id} className="border rounded-lg p-4">
                            <div className="flex justify-between items-center mb-2">
                                ...
                                <span className={`px-2 py-1 rounded text-xs ${course.completed ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                            ...
                            <div className="flex justify-between text-sm text-gray-500">
                                ...
                                {course.completed && <span>Completado: {course.date}</span>}
                            </div>
                            <div className="flex justify-end pt-2">
                                <Link href="/curso" className="rounded-md bg-slate-800 text-white p-2 w-[50px] text-center">
                                    Ver
                                </Link>
                            </div>
                        </div>
                ...
                <div className="p-6">
                    ...
                    {courses.filter(course => course.certificate).map(course => (
                        <div key={course.id} className="border rounded-lg p-4 flex flex-col">
                        <div className="flex justify-between items-center mb-2">
                            <h3 className="font-medium">{course.title}</h3>
                            <span className="text-sm text-gray-500">Fecha: {course.date}</span>
                        </div>
                        <div className="flex-grow flex items-center justify-center bg-gray-50 p-6 mb-4 rounded">
                            ...
                            <p className="text-sm mt-2 text-gray-600">Certificado #{course.certificate}</p>
                            </div>
                        </div>
                ...
                <div className="p-6">
                    ...
                    {courses.map(course => (
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            <span key={`${course.id}-${skill}`} className="bg-gray-100 text-gray-800 text-xs px-2 py-1 rounded">
                                {skill}
                            </span>
                            ))}
                        </div>
