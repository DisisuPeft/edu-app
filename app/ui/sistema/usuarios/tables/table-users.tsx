...
export default function TableUsers() {
  ...
  const columns: ColumnDef<User>[] = [
    ...
    },
    {
      accessorFn: (row) =>
        row.profile.nombre +
        " " +
        row.profile.apellidoP +
        " " +
        (row.profile.apellidoM ?? ""),
      id: "nombreCompleto",
        ...
        return (
          <div className="flex flex-wrap gap-1">
            {roles.map((r) => (
              <div
                key={r.id}
                className="bg-sky-500 text-white px-2 py-1 rounded text-xs font-medium"
              >
                {r.name}
              </div>
            ))}
  ...
  return (
    <div className="overflow-auto rounded-lg border border-gray-200 shadow-md">
      ...
      <UsersDialog open={open} setClose={handleClose}>
